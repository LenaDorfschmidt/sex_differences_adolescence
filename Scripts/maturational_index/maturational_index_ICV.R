...
lm.CV.m = lme(vol~mri_pseudo_age + mri_center, random=~1|id_nspn,data=subset(MRI_table, sex==1))
lm.CV.f = lme(vol~mri_pseudo_age + mri_center, random=~1|id_nspn,data=subset(MRI_table, sex==0))

CV.m = lm.CV.m$coefficients$fixed[1]+14*lm.CV.m$coefficients$fixed[2]+1/3*lm.CV.m$coefficients$fixed[3]+1/3*lm.CV.m$coefficients$fixed[4]
CV.f = lm.CV.f$coefficients$fixed[1]+14*lm.CV.f$coefficients$fixed[2]+1/3*lm.CV.f$coefficients$fixed[3]+1/3*lm.CV.m$coefficients$fixed[4]
group.glob.CV = list(CV.f, CV.m)
...
for (s in (1:2)) {
  ...
  for (n in 1:nroi) {
    ...
    try({
      ...
      str.l.14[s,n] = l$coefficients$fixed[1] + l$coefficients$fixed[2]*14 + l$coefficients$fixed[3]*(1/3) + l$coefficients$fixed[4]*(1/3) +  group.glob.CV[[s]] * l$coefficients$fixed[5] 
    })
    ...
    try({
      ...
      str.cort.l.14[s,n] = l$coefficients$fixed[1] + l$coefficients$fixed[2]*14 + l$coefficients$fixed[3]*(1/3) + l$coefficients$fixed[4]*(1/3) +  group.glob.CV[[s]] * l$coefficients$fixed[5] 
    ...
    try({
      ...
      str.subc.l.14[s,n] = l$coefficients$fixed[1] + l$coefficients$fixed[2]*14 + l$coefficients$fixed[3]*(1/3) + l$coefficients$fixed[4]*(1/3) +  group.glob.CV[[s]] * l$coefficients$fixed[5] 
  ...
  for (i in 1:nsub) {
    for (n in 1:nroi) {
      try({
        ...
        str.subc.cort.l.14[s,i,n] = l$coefficients$fixed[1] + l$coefficients$fixed[2]*14 + l$coefficients$fixed[3]*(1/3) + l$coefficients$fixed[4]*(1/3) +  group.glob.CV[[s]] * l$coefficients$fixed[5] 
  ...
  for (i in 1:(nroi-1)) { # only loop over upper triangular (as matrices are symmetric), and then fill lower triangular with the transpose
    ...
    for (j in (i+1):nroi) {
      possibleError <- tryCatch({
        ...
        fc.l.14[i,j] = l$coefficients$fixed[1] + l$coefficients$fixed[2]*14 + l$coefficients$fixed[3]*(1/3) + l$coefficients$fixed[4]*(1/3) +  group.glob.CV[[s]] * l$coefficients$fixed[5]
